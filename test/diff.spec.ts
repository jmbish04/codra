...
describe('filterReviewableFiles generated/migration handling', () => {
  ...
  it('keeps a modified file with @generated in context (unchanged) but new code without marker', () => {
    // This is a regression test: @generated in context lines should not filter the file.
    // Only markers in ADDED lines should trigger filtering.
    const modifiedDiff = `diff --git a/src/util.ts b/src/util.ts
--- a/src/util.ts
+++ b/src/util.ts
@@ -1,3 +1,4 @@
 // @generated by tool, do not edit manually
 export const old = 1;
+export const newFunc = () => 42;
 `;
    const files = parseUnifiedDiff(modifiedDiff);
    expect(filterReviewableFiles(files, cfg).map((f) => f.path)).toContain('src/util.ts');
  });
});
