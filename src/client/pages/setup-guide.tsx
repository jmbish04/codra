            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Codra uses two Durable Object classes: <strong>PrReviewStream</strong> (server-sent events for a live PR review) and <strong>JobsFeed</strong> (live jobs dashboard feed). These are declared in <code className="font-mono text-xs bg-muted/40 p-0.5 px-1 rounded">wrangler.jsonc</code> under <code className="font-mono text-xs bg-muted/40 p-0.5 px-1 rounded">durable_objects.bindings</code> — no manual provisioning is needed; Wrangler creates them on deploy. AI inference runs through the <strong>core-guardian</strong> service binding, not a Durable Object.
                </p>
                <InfoCallout variant="warning">
                  DO migrations are defined in <code className="font-mono text-xs bg-muted/40 p-0.5 px-1 rounded">wrangler.jsonc</code> under <code className="font-mono text-xs bg-muted/40 p-0.5 px-1 rounded">migrations</code>. When adding a new Durable Object class, add it to <code className="font-mono text-xs bg-muted/40 p-0.5 px-1 rounded">new_sqlite_classes</code> in a new migration tag. Existing classes: PrReviewStream (<code className="font-mono text-xs bg-muted/40 p-0.5 px-1 rounded">v2</code>), JobsFeed (<code className="font-mono text-xs bg-muted/40 p-0.5 px-1 rounded">v3</code>).
                </InfoCallout>
              </div>
            </div>
